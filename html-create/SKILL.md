...
- SVG（Scalable Vector Graphics：拡大しても劣化しないベクター画像形式）を作成するときは [SVG 作図ガイド](references/svg-diagrams.md) を読み、参考になる図があれば該当する詳細資料だけを読む
- 39種類は参考となる作図パターンであり、選択肢の上限ではない。文脈に合う独自の図や既存パターンの応用・組み合わせも作成する
- 共通ガイドの描画・数量・読み上げ規則を適用する。既存の図法を使う部分は、その記号・配線・数量表現の意味を保つ
- ノード数が多い場合は縦方向を優先する
- 矢印が交差する構成を避ける
