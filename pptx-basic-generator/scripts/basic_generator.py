...
テーマ、テンプレート、エンジン、設定データすべてをこの1ファイルに集約。
...
【設計思想】
================================================================================
- "Enhanced Charts": グラフの配色をプロフェッショナル仕様に刷新
  （青・ゴールド・グレーのグラデーション）
- 円グラフ（Pie Chart）のサポート追加
- XML直接操作による箇条書き（Native Bullets）も継続採用
- テンプレート再利用: 同じスライドデザインを異なる内容で複数回使用可能
- 可変コンテンツ: 箇条書き・グラフ・テーブルは配列サイズで自動調整
