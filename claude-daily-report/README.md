<h1 align="center">Claude Daily Report</h1>
...
Claude Code の会話履歴（`~/.claude/history.jsonl`）を解析し、日報作成のためのAgent Skillです。
...
Claude CodeのSkillsディレクトリにコピーしてください（既に存在する場合は上書きになります）。

```bash
cp -pr claude-daily-report ~/.claude/skills/daily-report/
```

Claude Code内で`/daily-report`を選択してください。
